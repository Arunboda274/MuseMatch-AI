...
def normalize_db(value: float, minimum: float = -60.0, maximum: float = 0.0) -> float:
    normalized = (value - minimum) / (maximum - minimum)
    return clamp(normalized)


def analyze_audio_file(file_path: str | Path) -> dict[str, float]:
    path = Path(file_path)
    ...
    rhythmic_strength = float(np.mean(onset_strength))
    danceability = clamp(
        0.55 * min(tempo / 180.0, 1.0)
        + 0.45 * min(rhythmic_strength / 4.0, 1.0)
    )
    ...
    total_component_energy = harmonic_energy + percussive_energy

    if total_component_energy > 0:
        instrumentalness = clamp(
            harmonic_energy / total_component_energy
        )
    else:
    ...
    speechiness = clamp(
        float(np.mean(zero_crossing_rate)) * 4
    )
    ...
    liveness = clamp(
        float(np.std(spectral_contrast)) / 20.0
    )
    ...
    valence = clamp(
        float(np.mean(chroma)) * 2.5
    )

    loudness = float(
        librosa.amplitude_to_db(
            np.array([max(average_rms, 1e-10)])
        )[0]
    )
